...
def _parse_s1_response(raw: str) -> S1Result:
    ...
    for e_raw in obj.get("entities", []):
        ...
        # The parser reads ``confidence`` when the LLM volunteers it and
        # defaults to 1.0 otherwise. qwen3.5:9b ignores the instruction;
        # stronger models can populate this to drive pending_review
        # behaviour downstream without any further code changes.
...
def _validate_s1(
    ...
    for e in ext.entities:
        ...
        # flagged the entity as low confidence (< 0.7). It's already
        # marked uncertain — we don't need a second filter telling it
        # off. qwen3.5:9b defaults confidence to 1.0 so this path is
        # dead code today, but activates automatically for any future
        # model that populates the field.
