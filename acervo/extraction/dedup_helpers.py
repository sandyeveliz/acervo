"""Deterministic dedup helpers: exact normalization + MinHash LSH fuzzy match.

Adapted from Graphiti (Apache-2.0, Zep Software).
See acervo/THIRD_PARTY.md for full attribution.

Original source:
    graphiti_core/utils/maintenance/dedup_helpers.py
    https://github.com/getzep/graphiti (graphiti-core 0.28.2)

Changes from upstream:
    - TYPE_CHECKING import points at our local DedupNode protocol instead of
      Graphiti's EntityNode, so this module has zero coupling to Graphiti runtime.
    - _FUZZY_JACCARD_THRESHOLD lowered from 0.9 to 0.85 to handle Spanish text
      with accents and typos.
...
_MIN_NAME_LENGTH = 6
_MIN_TOKEN_COUNT = 2
# Lowered from Graphiti's 0.9 to 0.85 — Spanish with accents and common typos
# (e.g. "cipolletti" vs "ciplinetti") scores in the 0.85-0.88 range.
...
class DedupCandidateIndexes:
    ...
    existing_nodes: list['DedupNode']
    nodes_by_uuid: dict[str, 'DedupNode']
    normalized_existing: defaultdict[str, list['DedupNode']]
...
class DedupResolutionState:
    ...
    resolved_nodes: list['DedupNode | None']
    uuid_map: dict[str, str]
    unresolved_indices: list[int]
    duplicate_pairs: list[tuple['DedupNode', 'DedupNode']] = field(default_factory=list)


def _promote_resolved_node(
    extracted_node: 'DedupNode',
    resolved_node: 'DedupNode',
) -> 'DedupNode':
    """Upgrade a generic canonical node when a duplicate carries a specific type.

    Acervo stores `type` as a single string (not a `labels: list[str]` like Graphiti),
    so this reduces to: if the resolved (existing) node is generic ("entity") and
    the extracted one has a specific type, promote the resolved node's type.
    """
    generic_types = {'entity', '', None}
    if resolved_node.type not in generic_types:
        return resolved_node

    if extracted_node.type not in generic_types:
        resolved_node.type = extracted_node.type

    return resolved_node


def _build_candidate_indexes(existing_nodes: list['DedupNode']) -> DedupCandidateIndexes:
    """Precompute exact and fuzzy lookup structures once per dedupe run."""
    normalized_existing: defaultdict[str, list['DedupNode']] = defaultdict(list)
    nodes_by_uuid: dict[str, 'DedupNode'] = {}
    shingles_by_candidate: dict[str, set[str]] = {}
...
def _resolve_with_similarity(
    extracted_nodes: list['DedupNode'],
    ...
    for idx, node in enumerate(extracted_nodes):
        ...
        best_candidate: 'DedupNode | None' = None
        best_score = 0.0
